...
class MultiSignal(gym.Env):
    def __init__(self, run_name, map_name, seed, net, state_fn, reward_fn, route=None, gui=False, end_time=3600,
        ...
        traci.close()
        self.connection_name = run_name + '-' + map_name + '-' + str(len(lights)) + '-' + state_fn.__name__ + '-' + reward_fn.__name__ + '-' + str(seed)
    ...
    def calc_metrics(self, rewards):
        ...
        for signal_id in self.signals:
            signal = self.signals[signal_id]
            queue_length, max_queue ,total_wait , max_wait= 0, 0, 0, 0
            for lane in signal.lanes:
                queue = signal.full_observation[lane]['queue']
                total = signal.full_observation[lane]['total_wait']
                mx = signal.full_observation[lane]['max_wait']
                total_wait += total
                max_wait += mx
                if queue > max_queue: max_queue = queue
                queue_length += queue
            queue_lengths[signal_id] = queue_length
            max_queues[signal_id] = max_queue
            total_waits[signal_id] = total_wait
            max_waits[signal_id] = max_wait
        self.metrics.append({
            'step': self.sumo.simulation.getTime(),
            'reward': rewards,
            'max_queues': max_queues,
    ...
    #     wait = []
    #     for ts in self.all_ts_ids:
    #         travel.append(self.signals[ts].get_travel_time())
    #         wait.append(self.signals[ts].get_waiting_time())
    #     self.metrics.append({
    #         'step_time': int(self.sumo.simulation.getTime()),
    #         'reward': rewards,
    #         'travel_time': int(np.mean(travel)),
    #         'waiting_time': int(np.mean(wait))
    #     })
    ...
    def save_metrics(self):
        ...
        with open(log, 'w+') as output_file:
            for line in self.metrics:
                csv_line = ''
                for metric in ['step', 'reward', 'max_queues', 'queue_lengths', 'total_wait', 'max_wait']:
    ...
    #     print(f'Saved episode {self.run}_th ')
