...
from pfrl.replay_buffer import AbstractEpisodicReplayBuffer, ReplayUpdater, batch_experiences, batch_recurrent_experiences
...
from pfrl.utils.recurrent import get_recurrent_state_at, mask_recurrent_state_at, one_step_forward, pack_and_forward, recurrent_state_as_numpy

def _mean_or_nan(xs: Sequence[float]) -> float:
    """Return its mean a non-empty sequence, numpy.nan for a empty one."""
    return np.mean(xs) if xs else np.nan

def compute_value_loss(y: torch.Tensor, t: torch.Tensor, clip_delta: bool = True, batch_accumulator: str = "mean",) -> torch.Tensor:
    """
...
def compute_weighted_value_loss( y: torch.Tensor, t: torch.Tensor, weights: torch.Tensor, clip_delta: bool = True, batch_accumulator: str = "mean",) -> torch.Tensor:
    """Compute a loss for value prediction problem.
    Args:
        y (torch.Tensor): Predicted values.
        t (torch.Tensor): Target values.
...
def _batch_reset_recurrent_states_when_episodes_end(batch_done: Sequence[bool], batch_reset: Sequence[bool], recurrent_states: Any) -> Any:
